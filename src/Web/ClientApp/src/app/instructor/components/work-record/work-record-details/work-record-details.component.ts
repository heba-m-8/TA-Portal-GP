...
import { InstructorClient, TaskDto, WorkRecordClient } from 'src/app/web-api-client';
...
export class WorkRecordDetailsComponent implements OnInit{
  ...
  constructor(private router: Router,private instructorClient: InstructorClient, private dialog: MatDialog , private workRecordClient: WorkRecordClient) { 
  ...
  approve(){
    this.instructorClient.approveWorkRecord24(this.userId ,this.items.id).subscribe((responce) => {
      this.router.navigate(['instructor/workrecord']);
    });
  }
