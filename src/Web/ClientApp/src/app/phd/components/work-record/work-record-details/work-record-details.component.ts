...
import { TaskDto, UpdateWorkRecordDto,TAClient, WorkRecordClient, UpdateTaskDto, PhdTAClient } from 'src/app/web-api-client';
...
export class WorkRecordDetailsComponent implements OnInit{
  ...
  totalHours:any;
  constructor(private router: Router,private TAClient: TAClient, private workRecordClient: WorkRecordClient,private phdTAClient: PhdTAClient) { 
  ...
  submit(){
    ...
    if(this.workRecordDetails.isSubmitted){
      this.phdTAClient.reSubmitWorkRecord(updateWorkRecordDto).subscribe((responce) => {
        this.router.navigate(['taphd/workrecord']);

      });
    }
    else{
      this.phdTAClient.submitWorkRecord(updateWorkRecordDto).subscribe((responce) => {
        this.router.navigate(['taphd/workrecord']);
