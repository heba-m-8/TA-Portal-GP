...
import { DeanOfGraduateStudiesClient, WorkRecordDto } from 'src/app/web-api-client';
...
export class RejectedWorkRecordComponent implements OnInit,AfterViewInit  {
  ...
  WorkRecordList:any[]=[];
  constructor(private deanOfGraduateStudiesClient: DeanOfGraduateStudiesClient,private router: Router ) {
  ...
  getWorkRecord(userId){
    this.deanOfGraduateStudiesClient.getRejectedWorkRecords2(userId).subscribe((responce) => {
  ...
  manageWorkRecord(data) {
    ...
    this.router.navigate(['deanofgraduatestudies/details',data.id], navigationExtras);
  }
}
