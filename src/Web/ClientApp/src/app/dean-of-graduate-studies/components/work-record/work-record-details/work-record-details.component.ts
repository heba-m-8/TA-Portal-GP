...
import { DeanOfGraduateStudiesClient, TaskDto, WorkRecordClient } from 'src/app/web-api-client';
...
export class WorkRecordDetailsComponent implements OnInit{
  ...
  constructor(private router: Router,private deanOfGraduateStudiesClient: DeanOfGraduateStudiesClient, private dialog: MatDialog , private workRecordClient: WorkRecordClient) { 
  ...
  approve(){
    this.deanOfGraduateStudiesClient.approveWorkRecord2(this.items.id).subscribe((responce) => {
      this.router.navigate(['/deanofgraduatestudies']);
    });
  }
