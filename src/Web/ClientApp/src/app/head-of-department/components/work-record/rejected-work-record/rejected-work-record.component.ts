...
import { HODClient, WorkRecordDto } from 'src/app/web-api-client';
...
export class RejectedWorkRecordComponent implements OnInit,AfterViewInit  {
  ...
  WorkRecordList:any[]=[];
  constructor(private hodClient: HODClient,private router: Router ) {
  ...
  getWorkRecord(userId){
    this.hodClient.getRejectedWorkRecords22(userId).subscribe((responce) => {
  ...
  manageWorkRecord(data) {
    ...
    this.router.navigate(['hod/workrecord/details',data.id], navigationExtras);
  }
}
