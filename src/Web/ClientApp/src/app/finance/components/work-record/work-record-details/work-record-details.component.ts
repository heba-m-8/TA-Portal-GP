...
import { FinanceClient, TaskDto, WorkRecordClient } from 'src/app/web-api-client';
...
export class WorkRecordDetailsComponent implements OnInit{
  ...
  constructor(private router: Router,private financeClient: FinanceClient, private dialog: MatDialog , private workRecordClient: WorkRecordClient) { 
  ...
  approve(){
    this.financeClient.approveWorkRecord22(this.items.id).subscribe((responce) => {
